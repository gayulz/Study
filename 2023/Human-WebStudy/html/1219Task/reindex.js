...
function addRow() {
  ...
  cartList.removeAttribute("hidden");
  if (document.querySelector("#hidden")) {
    document.querySelector("#hidden").setAttribute("hidden", "true");
  }
  getItems();
  addRowItems();
  editPrice();
...
function allChk1() {
  ...
  const isChkBox = document.querySelectorAll("#boxs");
  isChkBox.forEach(box => box.checked = allCheck.checked);
  editPrice();
}
...
function editPrice() {
  ...
  totalPrice.childNodes[9].innerHTML = `${innerTotal}`;
}
...
function addRowItems() {
  const cartTableBody = document.querySelector("#cart__list");
  const newRow = cartTableBody.insertRow();
  newRow.innerHTML = `
    <td><input type="checkbox" id="boxs" onchange="editPrice()" checked></td>
    <td>${itemInfoText}</td>
    <td><input type="number" class="quantity-input" min="1" value="${itemCnt}" name="${itemPrice}" 
               oninput="validateNumberInput(this)" style="width: 40px;">개</td>
    <td>${itemPrice * itemCnt}</td>
    <td>${itemDlv}</td>`;
}
...
function quantityChangeHandler(event) {
  ...
  const itemPrice = parseFloat(input.getAttribute("name"));
  const totalPrice = newQuantity * itemPrice;
  const totalCell = input.parentElement.nextElementSibling;
  totalCell.innerHTML = totalPrice.toFixed(2);
  editPrice();
}

function delRow() {
  document.querySelectorAll("#boxs:checked").forEach(box => {
...
function showHiddens() {
  if (document.querySelector("#cart__list").childElementCount === 0) {
